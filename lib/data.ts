 ...
 * four, 420 seats.
 *
 * HOLD, 4 Sep 2026: the owner reallocated the retired branch's 60 seats to
 * Computer Science (CSE 60 -> 120) and it briefly shipped, then ruled
 * "hold CSE at 60 until approval comes" — AICTE/BTEUP has not sanctioned
 * the move, and publishing 120 would state an intake above the approved
 * figure for that branch. So CSE stays 60 and the public total stays 420
 * until a revised EoA lands. When it does, the change is: CSE -> 120 here,
 * retired branch -> 0 (keeping SANCTIONED_SEATS at 480), the CMS branch row
 * in the same pass (app/llms.txt/route.ts fails the build otherwise), the
 * nine hardcoded public claims, and only then /approvals + the
 * /mandatory-disclosure intake sentence. Reverts of fb046ea and 26ad62b
 * carry the exact edit list. Those are two different true numbers and they are not
