...
/**
 * Best-effort: WhatsApp the BIPE admin number when a Meta Instant Form
 * (Lead Ad) submission arrives via the /api/meta-leads webhook. Speed-
 * to-lead is the whole point of that pipeline — a lead called within
 * minutes converts far better than one pulled from a CSV next day — so
 * this fires even when the backend ingest fails (the WhatsApp ping is
 * the safety net, not a nice-to-have).
 *
 * Configuration (mirrors fireAlumniIntroAdminNotification):
 *   DOUBLETICK_ADMIN_NUMBER               — required; admissions WhatsApp.
 *   DOUBLETICK_TEMPLATE_META_LEAD_ADMIN   — optional override; defaults
 *     to DEFAULT_TEMPLATE_NAME so the flow ships on the already-approved
 *     template. The lead summary rides the {{3}} placeholder.
 *
 * refSuffix should be the backend submission id when available so the
 * admin ping and the student's confirmation share one BIPE/ENQ/... ref.
 */
export function fireMetaLeadAdminNotification(args: {
  refSuffix: string;
  name: string;
  phone: string;
  email?: string;
  city?: string;
  branch?: string;
  /** "campaign · ad" attribution line from the webhook, if present. */
  campaign?: string;
}): void {
  const adminNumber = process.env.DOUBLETICK_ADMIN_NUMBER;
  if (!adminNumber) {
    console.warn(
      "[doubleTick] META LEAD RECEIVED but DOUBLETICK_ADMIN_NUMBER is missing — admin will NOT be pinged. Configure the env var; speed-to-lead depends on it.",
    );
    return;
  }

  const templateName =
    process.env.DOUBLETICK_TEMPLATE_META_LEAD_ADMIN || DEFAULT_TEMPLATE_NAME;

  const refId = buildReferenceId("enquiry", args.refSuffix);

  const summary = [
    `New Meta ad lead: ${args.name} (${args.phone})`,
    args.branch ? `Branch: ${args.branch}` : null,
    args.city ? `City: ${args.city}` : null,
    args.email ? `Email: ${args.email}` : null,
    args.campaign ? `Ad: ${args.campaign}` : null,
    "Call back fast — minutes beat hours on ad leads",
  ]
    .filter((p): p is string => !!p && p.length > 0)
    .join(". ")
    .replace(/\s+/g, " ")
  ...
  sendDoubleTickTemplate({
    ...
    .then((result) => {
      if (!result.ok) {
        console.warn(
          `[doubleTick] meta lead admin notification FAILED: ${result.error}`,
        );
      }
    })
    .catch((err) => {
      console.warn("[doubleTick] meta lead admin notification threw:", err);
    });
}
